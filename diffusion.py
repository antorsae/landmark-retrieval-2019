...
def similar_kernel(dot_product):
...
def dfs_trunk(sim, A, alpha=0.99, QUERYKNN=10, maxiter=8, K=100, tol=1e-3):
    ...
    for i in range(qsims.shape[0]):
        ...
        f, inf = linalg.minres(Wnn, qs[w_idxs], tol=tol, maxiter=maxiter)
...
def cg_diffusion(qsims, Wn, alpha=0.99, maxiter=10, tol=1e-3):
    ...
    out_sims = []
    for i in range(qsims.shape[0]):
        # f,inf = linalg.cg(Wnn, qsims[i,:], tol=tol, maxiter=maxiter)
        f, inf = linalg.minres(Wnn, qsims[i, :], tol=tol, maxiter=maxiter)
